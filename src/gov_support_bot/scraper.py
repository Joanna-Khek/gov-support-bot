...
class SchemesData:
    ...
    def _get_title(self):
        self.title = (
            self.scheme.find_element(By.CLASS_NAME, "MainSection-sc-k5m04n-2")
    ...
    def _get_tags(self):
        tags = self.scheme.find_element(
            By.CLASS_NAME, "TagsContainer-sc-k5m04n-9"
    ...
    def _get_category(self):
        time.sleep(1)
        categories = self.webdriver.find_element(
            By.CLASS_NAME, "category-wrapper"
        ).find_elements(By.TAG_NAME, "a")
        for cat in categories:
            self.category_list.append(cat.text)

    def _get_highlights(self):
        highlights = self.webdriver.find_element(
            By.CLASS_NAME, "SchemeHighlightsWrapper-sc-1el0czi-2"
    ...
    def _get_accordian_items(self):
        ...
        (
            self.webdriver.find_element(
                By.XPATH,
                "//button[contains(@class, 'StyledTextButton') and span[text()='Expand all sections']]",
            ).click()
        )

        self.accordian_items = self.webdriver.find_elements(
            By.CSS_SELECTOR, '[data-accordion-component="AccordionItemPanel"]'
        )

    def _get_benefits_info(self):
        benefit_items = self.accordian_items[0].find_elements(By.TAG_NAME, "p")
    ...
    def _get_eligibility_info(self):
        eligible_items = self.accordian_items[1].find_elements(By.TAG_NAME, "p")
    ...
    def _get_apply_info(self):
        apply_items = self.accordian_items[2].find_elements(By.TAG_NAME, "p")
