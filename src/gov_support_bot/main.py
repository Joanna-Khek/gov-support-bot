...
def human_feedback_tags(data, thread):
    with st.container(border=True):
      ...
      selected_tags = st.pills("Predicted Tags",
...
def human_feedback_categories(data, thread):
    with st.container(border=True):
      ...
      selected_categories = st.pills("Predicted Categories",
...
def main_conversation_ui(data, thread):
    ...
    if not st.session_state.predict_tags:
      if st.button("Submit") and query != "":
          ...
          st.write_stream(stream_response("Based on your query, these are the predicted tags. Please update the tags if needed."))
    ...
    if (not st.session_state.predict_categories) and st.session_state.predict_tags:
      st.write_stream(stream_response("Based on your query, these are the predicted categories. Please update the categories if needed."))
