...
def generate_queries_langchain(state: State):
    prompt = PromptTemplate(template=prompts.QUERY_TRANSFORM_PROMPT,
                            input_variables=["query"])
    
    generate_queries_chain = prompt | llm

    generated_queries_dict = generate_queries_chain.invoke({"query": state["query"],
    ...
    return {"multiple_queries": generated_queries_dict,
            "generate_count": state.get("generate_count", 0) + 1}
    
    
def predict_tags(state: State):
    prompt = PromptTemplate(template=prompts.TAGS_PROMPT,
                            input_variables=["query", "tags_list"])
    
    pred_tag_chain = prompt | llm
    
    pred_tag = pred_tag_chain.invoke({"query": state["query"], 
...
def predict_categories(state: State):
    prompt = PromptTemplate(template=prompts.CATEGORIES_PROMPT,
                            input_variables=["query", "categories_list"])
    pred_cat_chain = prompt | llm
    
    pred_cat = pred_cat_chain.invoke({"query": state["query"], 
...
def filter_categories(state: State):
    ...
    for intent, cat_docs in state["rank_results"].items():
        results[intent] = utils.filter_documents_by_category(cat_docs,
                                                             state["predicted_categories"])
...
def filter_tags(state: State):
    ...
    for intent, tag_docs in state["filtered_docs"].items():
        results[intent] = utils.filter_documents_by_tag(tag_docs,
                                                        state["predicted_tags"])
...
def generate_response(state: State):
    llm_prompt = PromptTemplate(template=prompts.RESPONSE_PROMPT,
                                input_variables=["context", "query"])
    
    generate_response_chain = llm_prompt | llm
    
    response = generate_response_chain.invoke({"query": state["query"],
...
def check_relevant_answer(state: State):
    ...
    prompt = PromptTemplate(template=prompts.RELEVANT_ANSWER_PROMPT,
                            input_variables=["query", "generated_response", "context"],
                            partial_variables={"format_instructions": parser.get_format_instructions()})

    check_relevance_chain = prompt | llm | parser

    response = check_relevance_chain.invoke({"query": state["query"],
                                "generated_response": state["answer"],
                                "context": state["context"]})
        
    if (response['relevance'] == "No") and (state["generate_count"] < state["max_generate_count"]):
